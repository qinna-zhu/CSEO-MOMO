function yhat = my_srgtsSHEPEvaluate(x, srgtSRGT)
%Function srgtsSHEPEvaluate is used to predict the response of a Shepard
%model. Thus, for example:
%
%     YHAT = srgtsSHEPEvaluate(X, SURROGATE): returns the response YHAT
%     predicted by the Shepard model SURROGATE at all X sites. X can be
...
for c1 = 1 : NbPointsTest
    yhat(c1,1) = vtechLSHEPVAL( x(c1,:)', ...
        srgtSRGT.NbVariables, srgtSRGT.NbPoints, ...
        srgtSRGT.P', srgtSRGT.T', ...
        srgtSRGT.SHEP_Beta, srgtSRGT.SHEP_Radii);
end
yhat=yhat';
return
